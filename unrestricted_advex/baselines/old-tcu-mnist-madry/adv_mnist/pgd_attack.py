...
class LinfPGDAttack:
  def __init__(self, model, x_input, y_input, epsilon, k, a, random_start, loss_func):
    ...
    if loss_func == 'xent':
      ...
      true_label_mask = tf.one_hot(y_input, 10, dtype=tf.float32)
      correct_logit = tf.reduce_sum(true_label_mask * logits, axis=1)
      wrong_logit = tf.reduce_max((1 - true_label_mask) * logits, axis=1)
      ...
      wrong_logit = tf.reduce_max(wrong_label_mask * logits, axis=1)

      loss = wrong_logit - correct_logit
    else:
      raise ValueError('Unknown loss function. Defaulting to cross-entropy')
